...
const minDeviceRegistrationTimeout = 6 * 1000; // 1 minute
 ...
 */
//device: que es?
//measurerments es el message

module.exports = async function (context, deviceId, measurements, timestamp) {
    try {
        if (deviceId) {
            if (!/^[a-zA-Z0-9-._:]*[a-zA-Z0-9-]+$/.test(deviceId)) {
        ...
        let cnnstring=await getDeviceConnectionString(context, deviceId);
        const client = Device.Client.fromConnectionString(cnnstring, DeviceTransport.Http);
        ...
        await client.open();
        console.log(`[HTTP] Sending telemetry for device: ${deviceId}, connectionString: ${cnnstring}`);
        await client.sendEvent(message);
        await client.close();
    } catch (e) {
        // If the device was deleted, we remove its cached connection string
        if (e.name === 'DeviceNotFoundError' && deviceCache[deviceId]) {
            delete deviceCache[deviceId].connectionString;
        }

        let errorMessage= `Unable to send telemetry for device ${deviceId}: ${e.message}`;
        console.log();
...
async function getDeviceConnectionString(context, deviceId) {
    if (deviceCache[deviceId] && deviceCache[deviceId].connectionString) {
        return deviceCache[deviceId].connectionString;
    }

    let deviceHub = await getDeviceHub(context, deviceId);
    const connStr = `HostName=${deviceHub};DeviceId=${deviceId};SharedAccessKey=${await getDeviceKey(context, deviceId)}`;
 ...
 */
async function getDeviceHub(context, deviceId) {
    const now = Date.now();
...
async function getDeviceKey(context, deviceId) {
    ...
    let iotcSasKey = context.iotcSasKey;
    // if(!iotcSasKey)
    // {
    //     //if the environmen variable is not present, retrieve from Key Vault
    //     console.log(`Retrieving IOT Central SAS Key from KeyVault: ${context.primaryKeyUrl}`);
    //     iotcSasKey = await context.getSecret(context, context.primaryKeyUrl)
    // }

    const key = crypto.createHmac('SHA256', Buffer.from(iotcSasKey, 'base64'))
        .update(deviceId)
        .digest()
        .toString('base64');
